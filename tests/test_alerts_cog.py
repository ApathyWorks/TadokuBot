...
with a fixed ``now`` so nothing depends on the wall clock or a live loop.
"""
...
CARD = leaderboard_card.LeaderboardCard(title="wrap", entries=[], footer="")
...
@pytest.fixture
def patched_card(monkeypatch):
    """Make build_period_leaderboard_card return a ready card by default."""
    builder = AsyncMock(return_value=(CONTEST, CARD))
    monkeypatch.setattr(leaderboard_cog, "build_period_leaderboard_card", builder)
    return builder
...
async def test_maybe_post_weekly_posts_when_period_advanced(patched_card):
    ...
    assert isinstance(channel.send.await_args.kwargs["file"], discord.File)
...
async def test_maybe_post_skips_when_disabled(patched_card):
    channel = _fake_channel()
    bot = _bot_with_channel(channel)
...
async def test_maybe_post_monthly_posts_previous_month(patched_card):
    channel = _fake_channel(cid=555)
    bot = _bot_with_channel(channel)
...
async def test_maybe_post_yearly_posts_final_standings(monkeypatch):
    ...
    yearend = AsyncMock(return_value=(CONTEST, CARD))
    monkeypatch.setattr(leaderboard_cog, "build_yearend_card", yearend)
...
async def test_maybe_post_marks_done_when_nothing_to_post(monkeypatch):
    monkeypatch.setattr(
        leaderboard_cog, "build_period_leaderboard_card", AsyncMock(return_value=(CONTEST, None))
    )
    channel = _fake_channel()
    bot = _bot_with_channel(channel)
...
async def test_run_due_alerts_posts_for_all_enabled_kinds(patched_card):
    channel = _fake_channel(cid=555)
    bot = _bot_with_channel(channel)
    config_store.set_guild_alert(999, "weekly", enabled=True, channel_id=555, last_period=[2020, 1])
...
async def test_post_sends_to_configured_channel():
    ...
    await cog._post(999, 555, b"PNGDATA")
...
async def test_post_is_a_noop_when_channel_id_is_none():
    ...
    await cog._post(999, None, b"PNGDATA")  # must not raise
